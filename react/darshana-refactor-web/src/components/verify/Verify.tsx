...
interface ComponentProps {
  ...
  onClose: () => void;
  tc: any;
  userRecruiter: IUserData | undefined;
  userTalent: DataUSer;
  setUserTalent?: any;
}
export const VerifyModal: FC<ComponentProps> = ({
  experiences,
  lang,
  onClose,
  tc,
  ...
  const sliceExperience = (experiences: Workplace[]) => {
    ...
    setUserTalent(experiencesProximal);
  };
  useEffect(() => {
  ...
  const onSubmitFormJob = async (data: any) => {
    ...
    const userTalentResponse = await getUserByIdResponse(
      userTalent.id.toString(),
    );
    sliceExperience(userTalentResponse?.data.workplaces as Workplace[]);
  ...
  return (
    <FormProvider {...methodsFormJob}>
      ...
      >
        <div>
          <h2 className={styles.title}> {tc("verify-talent-title")}</h2>
          <p className={styles.description}>
            {tc("verify-talent-description")}
