...
jest.mock('react-native/Libraries/Lists/SectionList', () => {
  ...
  return {
    ...
    default: jest.fn((props) => (
      <RN.default {...props} initialNumToRender={15} />
    )),
  }
})
...
describe('Home.gestion', () => {
  ...
  it('navigate to search screen', async () => {
    const navigate = jest.fn()
    mockUseNavigation.mockReturnValue({ navigate })
    const { getByA11yLabel } = render(<Home />)
    await act(async () => undefined)
    fireEvent.press(getByA11yLabel('Buscar'))
    expect(navigate).toBeCalledWith('Main', { screen: 'Search' })
  })
