...
type ColorName = ColorNames[number]
...
const colors = App.select<ColorModes>({
...
const lightColors = colorKeys.reduce((prevColors, color) => {
  const value = colors[color]
  if (!value) return { ...prevColors, [color]: palette['red-500'] }
  return { ...prevColors, [color]: value.light }
}, {}) as Record<ColorName, string>

const darkColors = colorKeys.reduce((prevColors, color) => {
  const value = colors[color]?.dark
...
const theme = createTheme({
  ...
  colors: {
    ...
    danger: App.key === 'elcomercio' ? '#E30000' : palette['red-500'],
  ...
  fontFamily: {
    display: App.select({
      elcomercio: Platform.OS === 'ios' ? 'NotoSerif' : 'serif',
      default:
        Platform.OS === 'ios'
    ...
    body: App.select({
      elcomercio: 'NotoSans',
      default: Platform.OS === 'ios' ? 'Avenir' : 'Roboto',
    }),
  },
  fontSize: {
    xxs: base * 0.625, // 10
