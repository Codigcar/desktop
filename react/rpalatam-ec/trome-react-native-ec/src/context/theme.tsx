...
const colorSchemeDefault = App.select({
  elcomercio: INITIAL_THEME,
  default: Appearance.getColorScheme() || INITIAL_THEME,
})

const dynamicModeIsEnabled = App.select({
  elcomercio: false,
  default: true,
})

export const ThemeProvider: React.FC = ({ children }) => {
  const [colorScheme, setColorScheme] = useState(colorSchemeDefault)
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORE_SCHEME).then((theme) => {
      if (!theme) return
      if (!dynamicModeIsEnabled) return
