...
interface Context {
  favorites: Story['id'][]
  fetchFavoritesStories: () => Promise<Story[]>
...
export const FavoritesProvider: React.FC = ({ children }) => {
  ...
  const fetchFavoritesStories = useCallback(async () => {
    Service.abort()
    const currentIds = Array.from(refIds.current).reverse()
    const idsFetched = refStories.current.map((story) => story.id)
    const storiesForFetch = currentIds.filter((id) => !idsFetched.includes(id))
