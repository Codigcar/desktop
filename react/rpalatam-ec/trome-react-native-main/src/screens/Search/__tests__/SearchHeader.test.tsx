...
describe('Search Header', () => {
  ...
  it('should hide keyboard when tap on black bg', async () => {
    const spyDismissKeyboard = jest.spyOn(Keyboard, 'dismiss')
    const { getByTestId } = render(<Component />)

    fireEvent(getByTestId('search-input'), 'onFocus')
    await act(async () => undefined)

    fireEvent.press(getByTestId('bg-search-history'))
    expect(spyDismissKeyboard).toBeCalled()
