module "this" {
  ...
  handler          = "functions/send-document/handler.handler"
  runtime          = "nodejs16.x"
  memory_size      = "128"
  timeout          = 60
  ...
  environment = {
    ...
    NOTIFICATIONS_EVENT_STORE_TABLE_NAME = data.aws_dynamodb_table.notifications.name
    ...
    MSK_TOPIC_CONTRACT_RELEASED          = "com.gcredicorp.matrix.contract.released.v1"
    MSK_TOPIC_DOCUMENT_CREATED           = "com.gcredicorp.matrix.contract.document-created.v1"
    MSK_TOPIC_NOTIFICATION_REQUEST       = "com.gcredicorp.matrix.notification.requested.v2"
    SECRETS_MANAGER_MSK_NAME             = data.aws_secretsmanager_secret.this.name
    CONTRACT_SUMMARY_EMAIL_TEMPLATE      = "email_attachment_card_contract_notification"
  }
}

resource "aws_lambda_event_source_mapping" "this1" {
  event_source_arn  = data.aws_msk_cluster.this.arn
  function_name     = module.this.lambda-function.main.arn
  topics            = ["com.gcredicorp.matrix.contract.released.v1"]
...
resource "aws_lambda_event_source_mapping" "this2" {
  event_source_arn  = data.aws_msk_cluster.this.arn
  function_name     = module.this.lambda-function.main.arn
  topics            = ["com.gcredicorp.matrix.contract.document-created.v1"]
  starting_position = "TRIM_HORIZON"
    ...
    "Statement" : [
      {
        ...
        "Resource" : [
          ...
          data.aws_dynamodb_table.notifications.arn,
