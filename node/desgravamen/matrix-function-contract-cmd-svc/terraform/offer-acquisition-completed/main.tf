module "this" {
  ...
  handler          = "functions/offer-acquisition-completed/handler.handler"
  runtime          = "nodejs16.x"
  memory_size      = "128"
  timeout          = 60
...
resource "aws_lambda_event_source_mapping" "this1" {
  ...
  topics            = ["com.gcredicorp.matrix.contract.account-created.v1"]
...
resource "aws_lambda_event_source_mapping" "this2" {
  event_source_arn  = data.aws_msk_cluster.this.arn
  function_name     = module.this.lambda-function.main.arn
  topics            = ["com.gcredicorp.matrix.contract.card-emitted.v1"]
  starting_position = "TRIM_HORIZON"
