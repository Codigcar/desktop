module "this" {
  ...
  handler          = "functions/document-requested/handler.handler"
  runtime          = "nodejs16.x"
  memory_size      = "128"
  timeout          = 60
  ...
  environment = {
    ENV                                          = var.env
    BUCKET_DOCUMENTSRV                           = data.aws_s3_bucket.contract_documents.bucket
    LEAD_VIEW_TABLE_NAME                         = data.aws_dynamodb_table.lead_view.name
    ...
    "Statement" : [
      {
        ...
        "Resource" : [
          data.aws_dynamodb_table.lead_view.arn,
