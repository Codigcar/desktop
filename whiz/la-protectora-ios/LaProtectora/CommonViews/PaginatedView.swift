    ...
    var body: some View {
        HStack(spacing: 9) {
            ForEach(0...pages - 1, id: \.self) { i in
                Circle()
                    .foregroundColor(self.currentIndex == i ? Color.blue : Color.clear)
                    .overlay(Circle().stroke(self.currentIndex == i ? Color.clear : Color(hex: "979797"), lineWidth: 1))
...
struct PaginatedView<Content: View>: View {
    ...
    init(_ count: Int, orientation: Orientation, alignment: Alignment = .center, currentIndex: Binding<Int>, dragToDismiss: DragToDismiss, backgroundColor: Color, isDragEnabled: Bool, @ViewBuilder content: @escaping (Int, Transform) -> Content) {
                       ...
                       if self.isDragEnabled {
                            withAnimation {
                                if let prev = self.previous {
                                    ...
                                    if ev {
                                        let velocityTreshold: CGFloat = UIScreen.main.bounds.width == 375 ? 125 : 200
