...
export default class IdProductComponent extends Component {
  render() {
    return (
      <View {...this.props} style={styles.view}>
        <View style={styles.headerOuterContainer1}>
          <View style={styles.titleContainer}>
            <Text style={styles.titleFont}>Nro. De Producto</Text>
            <TouchableOpacity onPress={async () => {
              await Clipboard.setString(this.props.NrAccount);
              Toast.show({
                title: '¡Número de producto copiado!',
                buttonText: 'Ok'
              })
            }}>
              <Icon
